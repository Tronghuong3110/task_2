...
const ConfigFileGenerator = (props) => {
    ...
    const convertData = (data) =>{
        ...
        const bodyComment = ["# IP của broker","#Client ID","#Clean session","# Thời gian đợi kết nối thành công","# broker sẽ tự động disconnect client khi không nhận được bất kì gói tin nào từ client trong vòng 1.5 * keepAlive","# Thông tin login broker (username, password, topic)","#ID của probe"]
        var configLines = "";
        for(let i=0;i<configData.length;i++){
            configLines+= bodyComment[i]+'\n'
            configLines+= configData[i].key +"="+ configData[i].value +'\n'
        }
        // configLines+= "# ID_Probe" +'\n'+ props.id;
