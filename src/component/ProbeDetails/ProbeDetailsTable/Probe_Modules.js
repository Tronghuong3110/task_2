...
const Probe_Modules = ({ id,status }) => {
    ...
    const getProbeModules = () => {
        fetch("/api/v1/probe/modules?idProbe=" + id + "&&name=&&status=")
    ...
    const checkDoneContinues =()=>{
        const interval = setInterval(()=>{
            ...
            if(checkValue == 1 || checkValue ==null){
                ...
                fetch("/api/v1/probeModule/check")
    ...
    const doActions = (arr,action) => {
        ...
        fetch("/api/v1/probeModule/action/" + action, options)
            .then(response => response.text())
            .then(data => {
                notify("Received request succesfully", 1)
            })
            .then(()=>{
                fetch("/api/v1/probeModule/"+action,options)
            })
            .catch(err => console.log(err))
        checkDoneContinues()
        getModuleContinues()
    ...
    const deleteModule = (id, userChoice) => {
        if (userChoice) {
            ...
            fetch("/api/v1/probeModule?id=" + stringParam, options)
                .then(response => response.json())
                .then(data => {
